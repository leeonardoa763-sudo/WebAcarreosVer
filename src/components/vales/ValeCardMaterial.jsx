 ...
 *
 * Tarjeta compacta de vale de MATERIAL — abre modal centrado al hacer clic.
 *
 * Funcionalidades:
 * - Vista compacta densa: folio, operador, placas, estado, fecha, costo total
...
const ValeCardMaterial = ({ vale, empresaColor, onValeActualizado }) => {
  ...
  const { fecha } = formatearFechaHora(vale.fecha_creacion);

  const calcularCostoTotal = useCallback(() => {
    if (!vale.vale_material_detalles?.length) return 0;
